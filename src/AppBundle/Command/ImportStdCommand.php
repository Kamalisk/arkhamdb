...
class ImportStdCommand extends ContainerAwareCommand
{
	...
	protected function getEntityFromData($entityName, $data, $mandatoryKeys, $foreignKeys, $optionalKeys)
	{
		...
		if($entity->serialize() !== $orig || (isset($data['back_link']) && (!$entity->getLinkedTo() || $entity->getLinkedTo()->getCode() != $data['back_link'])) || (isset($data['bonded_to']) && (!$entity->getBondedTo() || $entity->getBondedTo()->getCode() != $data['bonded_to']))) return $entity;
	}
